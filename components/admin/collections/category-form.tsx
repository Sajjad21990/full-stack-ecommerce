...
import { createCategory, updateCategory, isCategoryHandleAvailable } from '@/lib/admin/actions/collections'
...
export function CategoryForm({ category, categories, parentId }: CategoryFormProps) {
  ...
  const onSubmit = async (data: CategoryFormData) => {
    ...
    try {
      ...
      if (!isEditing || data.handle !== category.handle) {
        const isAvailable = await isCategoryHandleAvailable(data.handle, category?.id)
        if (!isAvailable) {
  ...
  const buildCategoryTree = (cats: Category[], level = 0): JSX.Element[] => {
    return cats
      .filter(cat => (level === 0 ? !cat.parentId : cat.parentId === cats.find(p => p.level === level - 1)?.id))
      .map(cat => (

...
// Rate limiting analytics removed - feature disabled
...
export async function GET(request: NextRequest) {
  try {
    ...
    // Get rate limiting analytics (disabled)
    const rateLimitData = { violations: [], stats: { webhook: 0 } }
...
function calculateSuccessRate(fraudData: any): number {
  ...
  const failedTransactions =
    (fraudData.fraudBlocked || 0) + (fraudData.fraudDetected || 0)
  return (
    Math.round((1 - failedTransactions / fraudData.totalAnalyses) * 100 * 100) /
    100
  )
}
...
function formatIpAnalytics(violations: any[]): any[] {
  const ipStats = violations.reduce(
    (acc, violation) => {
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (
      !session?.user ||
      !hasPermission(session.user, 'admin:security:manage')
    ) {
      return NextResponse.json({ error: 'Unauthorized' }, { status: 401 })
    }
